...
def im2col(mtx,mtx2, block_size):
    ...
    for i in range(sy):
        for j in range(sx):
            ...
            result2[:, i * sx + j] = mtx2[j:j + block_size[0], i:i + block_size[1]].ravel(order='F')
...
def denoise_LO (un,vn, median_filter_size, lambda23, niters):
    ...
    #tmp=tmp.T
    tmpu = np.matlib.repmat( np.reshape(un,(1,un.shape[0]*un.shape[1]),'F') ,int(2*n+1), 1)+tmp
    tmpv = np.matlib.repmat( np.reshape(vn,(1,un.shape[0]*un.shape[1]),'F') ,int(2*n+1), 1)+tmp
    uo  = un
    vo=vn
    for i in range(niters):
        ...
        v2 = np.vstack((v2[:math.floor(mfsize*mfsize/2),:], v2[math.ceil(mfsize*mfsize/2): v2.shape[0], :]))
        uo=np.reshape(                      np.median( np.vstack(  (u2,tmpu))                     ,0), (un.shape[0],un.shape[1]),'F'      )
